...
### BFC(Block formatting context) 块级格式化上下文， 渲染区域概念

直译为"块级格式化上下文"。它是一个**独立的渲染区域**，只有Block-level box参与， 它规定了内部的**Block-level Box如何布局，并且与这个区域外部毫不相干。**

- display 属性为 block, list-item, table 的元素，会产生BFC。
- 布局特性：有宽度和高度 ， 有 外边距margin 有内边距padding 有边框 border。
- 不能理解为单个元素为一个BFC，两个元素合起来可以为一个BFC，每个都是一个独立的盒子，但是同属于一个BFC。
...
这里可以看出，BFC这个概念要**大于等于一般的盒子模型的单个元素的**，是愉快渲染区域。

#### BFC元素所具有的特性

1. 在BFC中，盒子从顶端开始**垂直地一个接一个地排列**.
...
- 清除元素内部浮动, overflow: hidden样式;
- 解决外边距合并问题; 盒子垂直方向的距离由margin决定，属于同一个BFC的两个相邻盒子的margin会发生重叠，不属于统一BFC的相邻盒子不会margin重叠