  ...
  <div class="img-upload-container inline-block">
    ...
    >
      <input
        ...
        @change="handleUpload"
      >
      <div class="img-add w100 h100 align-center">
        <i class="el-icon-plus pointer" />
        <div class="upload-notice w100">
...
import { uploadImg, deletImg } from '@/api/information'
  ...
  methods: {
    ...
    handleUpload(event) {
      ...
      uploadImg(formData).then(res => {
        if (res && +res.code === 200) {
          this.imgFiles = this.imgFiles.concat(res.object)

          this.$emit('uploadChange', this.imgFiles)
          this.previewImg(res.object)
        }
      })
    }
  }
