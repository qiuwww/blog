...
export default {
  ...
  watch: {
    ...
    //监听表单配置的表单选择
    "frender.formPropsData.tableName":{
      deep:true,
      handler:function(newV,oldV){
        if(newV){
          let _this = this
          getFieldData(newV).then(res => {
            let data =[]
            if (res &&res.content&& res.content.length>0) {
              for(let i in res.content){
                if(res.content[i] && res.content[i].columnName && res.content[i].columnName.trim()!="id"){
                  let item = {
                    text:res.content[i].columnName+"-"+res.content[i].remark,
                    value:res.content[i].columnName
                  }
                  data.push(item);
                }
              }
              _this.frender.formItemCommon.config.field.options = data;
              _this.frender.formItemCommon.config.field["required"] = false;
              // 当前组件信息
              this.formDesc = changeFormDescLabel({
                ...this.frender.formItemCommon.config,
                ...this.customConfig
