...
<!--       <Input v-model="formName" placeholder="输入保存的表单名称" clearable style="width: 200px" />-->
        <el-button @click="saveOption()" icon="el-icon-upload2" type="text"
            >保存数据</el-button
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    saveOption:function () {
      this.frender.fatherMethod(this.formName);
      this.$emit('save');
    }
