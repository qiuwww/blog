    ...
    export default {
        ...
        props: {
            ...
            rightTabs: {
                type: Array,
                default: () => [
                    {label: "表单配置", name: "form-props"}, {label: "表单项属性配置", name: "form-item-common"},
        ...
        watch: {
            config: {
                handler(config) {
                    ...
                    //判断是否是新添加页面 wangmaogeng 20210125
                    if(JSON.stringify(formConfig) === '{}'){
                      this.formPropsData = JSON.parse(JSON.stringify(this.formProps.data)) ;
                    }else
                    {
                      //edit by dengjie 20201124 添加返现的样式
                      this.formPropsData = JSON.parse(JSON.stringify(formConfig)) ;
                    }
        ...
        methods: {
            ...
            initFormPropsData() {
              ...
              this.formPropsData.inline = true;
            },
        },
        created() {
            // 设置表单属性默认值
