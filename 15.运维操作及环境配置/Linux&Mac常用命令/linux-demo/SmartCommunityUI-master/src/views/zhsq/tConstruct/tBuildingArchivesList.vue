...
<!--          </Form-item>-->
          <Form-item label="建筑用途">
            <Select class="select-form" v-model="searchForm.buildingType" placeholder="请选择"
                    style="width: 200px" @on-change="getDataList()" clearable>
              <Option v-for="(item, i) in buildingTypeData" :key="Math.random()" :value="item.number.toString()">
                {{ item.name }}
              </Option>
            </Select>
          </Form-item>
          ...
          <Form-item label="所属网格" v-show="drop">
            <Select v-model="searchForm.grid" placeholder="请选择所属网格" style="width: 200px"  clearable @on-change="getDataList()" clearable>
              <Option v-for="(item, i) in gridData" :key="item.id" :value="item.id" >
                {{ item.name }}
              </Option>
            </Select>
          </Form-item>
          ...
          <Form-item :label-width="40">
<!--            <Button v-if="permitIds.includes('archive_domain_sheshi_add')" @click="handleSearch" type="primary"-->
<!--                    icon="ios-search">搜索-->
<!--            </Button>-->
...
<!--          <span style="float: right;">点击右上角按钮配置动态列↑</span>-->
        ...
        <Table :loading="loading" stripe :columns="columns" sortable="custom" :data="data" @on-sort-change="changeSort"
          ...
          <template slot-scope="{ row, index }" slot="action">
            <Tooltip content="查看详情" placement="top">
              <Button  class="btnPrimary"  icon="md-eye"   @click="handleLook(row, index)" v-if="permitIds.includes('archive_domain_sheshi_see')"></Button>
            </Tooltip>
            <Tooltip content="修改" placement="top">
              <Button  class="bianji"   @click="handleEdit(row, index)" v-if="permitIds.includes('archive_domain_sheshi_bianji')"></Button>
            </Tooltip>
            <Tooltip content="删除" placement="top">
              <Button  class="btnError"  icon="ios-trash"   @click="deleteData(row, index)" v-if="permitIds.includes('archive_domain_sheshi_delete')"></Button>
            </Tooltip>
          </template>
        </Table>
      </Row>
    ...
    <lookConstruct v-model="lookShow" :TBuildingArchivesId="TBuildingArchivesId"
                          :modalTitle="title"></lookConstruct>
...
import lookConstruct from './lookConstruct'
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.searchForm.grid = "";
      this.gridData = [];
      this.searchForm.buildingType = '2,3,4,5,6,7,8,9';
    ...
    getTBuildingArchivesColumns() {
      return [
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.buildingName) {
              // let buildingName = params.row.housingEstate + params.row.buildingName ;
        ...
        },
        {
          ...
          minWidth: 100,
          render: (h, param) => {
            if (param.row.buildingType) {
              let tempArr = this.buildingTypeData.filter(item => item.number.toString() === param.row.buildingType.toString());
              let dataF = this.cvData.filter(item => item.id == param.row.id);
              dataF[0].buildingType = '暂无';
              if (tempArr.length > 0) {
                dataF[0].buildingType = tempArr[0].name;
    ...
    //table数据
    getTableData() {
      if (this.searchForm && (this.searchForm.buildingType == undefined || this.searchForm.buildingType == null || this.searchForm.buildingType.trim().length == 0)) {
        this.searchForm.buildingType = '2,3,4,5,6,7,8,9';
      }
