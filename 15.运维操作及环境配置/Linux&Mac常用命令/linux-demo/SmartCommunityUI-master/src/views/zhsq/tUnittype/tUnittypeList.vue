...
<template>
	<div class="search" tUnittypeList>
		<Card>
          ...
          <Form-item label="创建时间">
			...
			<Row class="operation">
				<Button @click="addTUnittype" type="primary" icon="md-add">新增</Button>
...
<!--                </div>-->
		...
		<updateTUnittype v-model="updateShow" :addFlag="addFlag" :TUnittypeId="TUnittypeId" :modalTitle="title" v-on:handleSearch="getDataList"></updateTUnittype>
	</div>
</template>
<script>
	import {
    ...
    queryTUnittypeList,
    queryTUnittypeTree,
    updateTUnittype as updateTUnittypeShow,
	} from '@/api/zhsq/tUnittype/tUnittype'
	import { formartDate } from '@/api/tools/tool'
	import updateTUnittype from './updateTUnittype'
	export default {
		components: {
			updateTUnittype
		},
		data() {
			return {
				...
				search: '',
				data: [],
				columns: this.getTUnittypeColumns(),
        ...
        tUnittypeForm: null,
				...
				updateShow: false,
				tableHeight: 200,
				colOptions: ["类型名称","1类别-0分类","显示状态","代码","排序码","创建时间","操作"],
				colSelect: ["类型名称","1类别-0分类","显示状态","代码","排序码","创建时间","操作"],
				csvData: [],
        /*分页查询单位(树级)*/
        pageNumber: 1, // 当前页数
        pageSize: 10, // 页面大小
        ...
        this.addFlag = true;
			...
			this.title = '编辑';
        this.addFlag = true;
				this.TUnittypeId=row.id.toString();
       ...
       handleAdd(row, index) {
        ...
        if(row.pid.toString() != '0'){
          this.addFlag = false;
			...
			getDataList() {
				...
				/*queryTUnittypeList(this.searchForm).then(res => {
					if(res && res.success) {
						this.loading = false;
        ...
        rows.forEach(function(row) {
          if (row.children && row.children.length > 0) {
            ...
            _this.$Modal.remove();
            _this.$Message.warning(row.name+'下仍有子级单位，不可删除');
            flag = false;
            return;
          }
        });
        if(flag) {
          deleteTUnittype({ids: ids}).then(res => {
            this.userLoading = false;
            this.$Modal.remove();
            if (res && res.success) {
       ...
       displayState(row) {
         ...
         this.tUnittypeForm = row;
         this.$Modal.confirm({
           title: "确认要" + str,
           content: "您是否要" + str + "?",
           loading: true,
           onOk: () => {
             updateTUnittypeShow(this.tUnittypeForm).then(res => {
               this.$Modal.remove();
               if (res && res.code == 200) {
					...
					},
          {
			...
			checkboxChange:function (data) {
				var columnss =  this.getTUnittypeColumns();
				this.columns= columnss.filter(function(v){ return data.indexOf(v.title) > -1 || v. type== 'selection'})
			}
		},
		mounted() {
			this.getDataList();
			//this.tableHeight = window.innerHeight - this.$refs.tUnittypeTable.$el.offsetTop - 270
		}
	}
</script>
<style scoped="less">
