    ...
    methods: {
      ...
      handelSubmit() {
        this.$refs['basicHousingManageForm'].validate((valid) => {
          if (valid) {
            if (this.tableName == 'tZhsqMerchantProfile') {
              let data = {}

              data.id = this.BasicId
              data.merchantAddress = this.basicHousingManageForm.houseGrid.trim() + this.basicHousingManageForm.houseName.trim() + '小区' + this.basicHousingManageForm.buildArchiveName.trim() + '号楼' + this.basicHousingManageForm.unit.trim() + '单元' + this.basicHousingManageForm.floor.trim() + this.basicHousingManageForm.doorNumberName.trim(),
