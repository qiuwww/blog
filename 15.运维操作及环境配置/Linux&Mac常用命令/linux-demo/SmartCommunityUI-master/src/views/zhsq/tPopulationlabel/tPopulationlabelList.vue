<style lang="less">
.tPopulationlabelList{
}
</style>
<template>
	<div class="search" tPopulationlabelList>
		<Card>
          ...
          <FormItem label="类型名称" prop="name">
			...
			<Row class="operation">
				<Button @click="addTPopulationlabel" type="primary" icon="md-add">新增</Button>
			...
			<Row>
				<Table :loading="loading" border :columns="columns" sortable="custom" row-key="id" :data="data" @on-sort-change="changeSort" @on-selection-change="showSelect" ref="table">
		...
		<updateTPopulationlabel v-model="updateShow" :addFlag="addFlag" :TPopulationlabelId="TPopulationlabelId" :modalTitle="title" v-on:handleSearch="getDataList"></updateTPopulationlabel>
	</div>
</template>
<script>
	import {
	  deleteTPopulationlabel,
    queryTPopulationlabelList,
    queryTPopulationlabelTree,
    updateTPopulationlabel as updateTPopulationlabelShow
	} from '@/api/zhsq/tPopulationlabel/tPopulationlabel'
	import { formartDate } from '@/api/tools/tool'
	import updateTPopulationlabel from './updateTPopulationlabel'
	export default {
		components: {
		...
		data() {
			return {
				...
				search: '',
				data: [],
				columns: this.getTPopulationlabelColumns(),
				...
				updateShow: false,
				tableHeight: 200,
				colOptions: ["类型名称","显示状态","创建时间","操作"],
				colSelect: ["类型名称","显示状态","创建时间","操作"],
				csvData: [],
        tPopulationlabelForm: null,
        /*分页查询标签(树级)*/
        pageNumber: 1, // 当前页数
        pageSize: 10, // 页面大小
        queryName: '', // 名称 模糊查询
        ...
        this.addFlag = true;
				...
				this.TPopulationlabelId='';
			},
			//编辑
			handleEdit(row, index) {
			this.title = '编辑';
        this.addFlag = true;
				this.TPopulationlabelId=row.id.toString();
       ...
       handleAdd(row, index) {
        ...
        if(row.pid.toString() != '0'){
          this.addFlag = false;
			...
			getDataList() {
				...
				/*queryTPopulationlabelList(this.searchForm).then(res => {
					if(res && res.success) {
						this.loading = false;
        ...
        rows.forEach(function(row) {
          if (row.children && row.children.length > 0) {
            ...
            _this.$Modal.remove();
            _this.$Message.warning(row.name+'下仍有子级标签，不可删除');
            flag = false;
            return;
          }
        });
        if(flag) {
          deleteTPopulationlabel({ids: ids}).then(res => {
            this.userLoading = false;
            this.$Modal.remove();
            if (res && res.success) {
       ...
       displayState(row) {
         ...
         this.tPopulationlabelForm = row;
         this.$Modal.confirm({
           title: "确认要" + str,
           content: "您是否要" + str + "?",
           loading: true,
           onOk: () => {
             updateTPopulationlabelShow(this.tPopulationlabelForm).then(res => {
               this.$Modal.remove();
               if (res && res.code == 200) {
          ...
          },
          {
			...
			checkboxChange:function (data) {
				var columnss =  this.getTPopulationlabelColumns();
				this.columns= columnss.filter(function(v){ return data.indexOf(v.title) > -1 || v. type== 'selection'})
			}
		},
		mounted() {
			this.getDataList();
			//this.tableHeight = window.innerHeight - this.$refs.tPopulationlabelTable.$el.offsetTop - 270
		}
	}
</script>
<style scoped="less">
