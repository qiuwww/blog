...
<template>
  <div class="search tZhsqAnounceInfoList" >
    <Card>
      <Row v-show="openSearch" @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="70" @submit.native.prevent>
          <Form-item label="公告标题">
            <Input v-model="searchForm.infoTitle" placeholder="请输入标题"></Input>
          </Form-item>
          <FormItem label="公告类型">
            <Select v-model="searchForm.infoType" placeholder="请选择" style="width: 213px" transfer>
...
<!--            </a>-->
      ...
      <Row class="operation">
        <Button @click="addTZhsqAnounceInfo" type="primary" icon="md-add">新增</Button>
    ...
    <updateTZhsqAnounceInfo v-model="updateShow" :TZhsqAnounceInfoId="TZhsqAnounceInfoId" :modalTitle="title"
                            v-on:handleSearch="getDataList"></updateTZhsqAnounceInfo>
    <look-zhsq-anounce-info v-model="lookShow" :tZhsqAnounceInfoId="TZhsqAnounceInfoId" modalTitle="公告查看" v-on:handleSearch="getDataList"></look-zhsq-anounce-info>
  </div>
</template>
<script>
import {deleteTZhsqAnounceInfo, queryTZhsqAnounceInfoList,updateTZhsqAnounceInfo as update} from '@/api/zhsq/tZhsqAnounceInfo/tZhsqAnounceInfo'
import {formartDate} from '@/api/tools/tool'
import updateTZhsqAnounceInfo  from './updateTZhsqAnounceInfo'
import lookZhsqAnounceInfo  from './lookZhsqAnounceInfo'
import {
  queryAllTZhsqAnnounceTypeList
} from '@/api/zhsq/tZhsqAnnounceType/tZhsqAnnounceType'
export default {
  components: {
    updateTZhsqAnounceInfo,lookZhsqAnounceInfo
  },
  data() {
    return {
      isShow: false,
      ...
      search: '',
      data: [],
      columns: this.getTZhsqAnounceInfoColumns(),
      ...
      updateShow: false,
      tableHeight: 200,
      colOptions: ["公告标题","公告类型", "是否隐藏", "公告内容",  "发布日期", "发布单位", "操作"],
      colSelect: ["公告标题","公告类型", "是否隐藏", "公告内容",  "发布日期", "发布单位", "操作"],
      csvData: [],
      infoTypeData:[],//公告类型集
      lookShow:false,//查看弹框是否显示
    }
  },
  methods: {
    /**
    ...
    addTZhsqAnounceInfo() {
      ...
      this.TZhsqAnounceInfoId = '';
    },
    //编辑
    handleEdit(row, index) {
      this.title = '编辑';
      this.TZhsqAnounceInfoId = row.id.toString();
      this.updateShow = true;
    },
    //查看
    ...
    getDataList() {
      ...
      queryTZhsqAnounceInfoList(this.searchForm).then(res => {
        if (res && res.success) {
          this.loading = false;
    ...
    patchdeleteData(ids) {
      ...
      deleteTZhsqAnounceInfo({ids: ids}).then(res => {
    ...
    updateData(row) {
      ...
      // console.log(params.row.announceShow)
      let data = {
        infoShow: row.infoShow,
        id: row.id,
      }
      let _this = this;
      //改变开关状态
    ...
    handleReset() {
      ...
      this.searchForm.infoType = null;
      this.searchForm.infoTitle = null;
    ...
    getTZhsqAnounceInfoColumns() {
      ...
      return [
        ...
        },
        {
          ...
          sortable: false,
          render: (h, params) => {
            let newTime =params.row.tzhsqAnnounceType.typeName;
    ...
    checkboxChange: function (data) {
      var columnss = this.getTZhsqAnounceInfoColumns();
  ...
  mounted() {
    ...
    //this.tableHeight = window.innerHeight - this.$refs.tZhsqAnounceInfoTable.$el.offsetTop - 270
  }
}
</script>
