  ...
  <div>
    <Modal v-model="show" :title="modalTitle" :mask-closable="false" class="personDownload" :draggable="false"
      ...
      <div slot="footer">
        ...
        <Button type="primary" :loading="loading" @click="handelSubmit">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handelSubmit(){
        this.$parent.colSelect = this.selectField
        this.$parent.checkboxChange(this.selectField);
    ...
    watch: {
      ...
      show(val) {
        this.initForm()
        this.selectField = this.$parent.colSelect
        if (val) {
        }
      }
    },
