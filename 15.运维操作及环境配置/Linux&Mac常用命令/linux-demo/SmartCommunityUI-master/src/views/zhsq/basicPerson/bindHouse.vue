  ...
  <Modal v-model="show" :title="editTitle" :mask-closable="false" class="basicHousingManage" width="991">
    <Form ref="basicHousingManageForm" :model="basicHousingManageForm" :rules="basicHousingManageFormRule">
      <Row>
        <Col span="8">
          <FormItem label="所属街道" prop="streetId">
            <Select
              v-model="basicHousingManageForm.streetId"
              placeholder="请选择"
              label-in-value
        ...
        <Col span="8">
          <FormItem label="所属社区" prop="communityId">
            <Select
              v-model="basicHousingManageForm.communityId"
              placeholder="请选择"
              label-in-value
        ...
        <Col span="8">
          <FormItem label="所属网格" prop="houseGridId">
            <Select
              v-model="basicHousingManageForm.houseGridId"
              placeholder="请选择"
              label-in-value
              @on-change="gridChange"
            >
              <Option v-for="(item, i) in gridData" :key="item.id.toString()" :value="item.id.toString()">{{ item.name
                }}
              </Option>
            </Select>
          </FormItem>
      ...
      <Row>
        <Col span="8">
          <FormItem label="小区名称" prop="houseId">
            <Select v-model="basicHousingManageForm.houseId" placeholder="请选择" label-in-value @on-change="houseChange">
              <Option v-for="(item, i) in houseName" :key="item.id.toString()" :value="item.id.toString()">{{ item.name
                }}
              </Option>
            </Select>
          </FormItem>
        ...
        <Col span="8">
          <FormItem label="楼栋" prop="buildArchiveId">
            <Select
              v-model="basicHousingManageForm.buildArchiveId"
              placeholder="请选择"
              label-in-value
              @on-change="buildArchivesChange"
            >
              <Option v-for="(item, i) in buildArchives" :key="item.id" :value="item.id.toString()">{{ item.buildingName
                +"栋"}}
              </Option>
            </Select>
          </FormItem>
        ...
        <Col span="8">
          <FormItem label="单元" prop="unit">
            <Select v-model="basicHousingManageForm.unit" placeholder="请选择" label-in-value @on-change="unitChange">
      ...
      <Row>
        <Col span="8">
          <FormItem label="楼层" prop="floor">
            <Select v-model="basicHousingManageForm.floor" placeholder="请选择" label-in-value @on-change="floorChange">
        ...
        <Col span="8">
          <FormItem label="门牌号" prop="doorNumber">
            <Select
              v-model="basicHousingManageForm.doorNumber"
              placeholder="请选择"
              label-in-value clearable
              @on-change="doorNumberChange"
            >
              <Option v-for="(item,i) in doorNumberData" :key="i" :value="item.id.toString()">{{ item.doorNumber }}</Option>
            </Select>
          </FormItem>
        </Col>
        <Col span="8">
          <FormItem label="与房主关系" prop="relationShip">
            <Select v-model="basicHousingManageForm.relationShip" placeholder="请选择" label-in-value>
              <Option v-for="(item,i)  in relationShipData" :key="i" :value="item.number.toString()">{{ item.name }}
              </Option>
            </Select>
          </FormItem>
        </Col>
      ...
      <Row>
        ...
        <!--        </FormItem>-->
        <!--        </Col>-->
        <Col v-if="rentStatusShow" span="8">
          <FormItem label="住房状态" prop="rentStatus">
            <Select v-model="basicHousingManageForm.rentStatus" placeholder="请选择">
              <Option v-for="(item ,i) in rentStatusData" :key="i" :value="item.number.toString()">{{ item.name }}
              </Option>
            </Select>
          </FormItem>
        </Col>
    ...
    methods: {
        ...
        getBasicPerson({id: this.BasicPersonId}).then(res => {
          if (res && res.success) {
            ...
            if (res.data.personType == '3') {
              ...
              this.relationShipData = this.relationShipData.filter(function (item, index) {
      ...
      doorNumberChange(e) {
        if (e) {
          ...
          this.basicHousingManageForm.id = this.BasicPersonId// 人口id
        }
        else{
          this.basicHousingManageForm.doorNumber = ""
          this.basicHousingManageForm.ownedHousing = ""// 房屋id
          this.basicHousingManageForm.id = ""// 人口id
        }
      },
      // 楼层改变
      floorChange(e) {
