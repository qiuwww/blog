  ...
  <div class="partyWrap">
    <div class="searchWrap">
      <Form ref="searchForm" :model="searchForm" inline :label-width="110" on-submit="return false">
        <Button v-if="permitIds.includes('dangjian_affairs_public_add')" type="primary" icon="md-add" @click="onAdd">新增</Button>
        ...
        <Form-item label="分类" :label-width="70">
          <el-select v-model="searchForm.partyCategoryId" placeholder="请选择分类">
            <el-option v-for="(item, i)  in categorys" :key="item.partyCategoryId" :label="item.partyCategoryName" :value="item.partyCategoryId"></el-option>
          </el-select>
        </Form-item>
        ...
        >
          <template slot="action" slot-scope="{ row, index }">
            <Tooltip content="查看详情" placement="top">
              <Button v-if="permitIds.includes('dangjian_affairs_public_see')" class="btnPrimary" icon="md-eye" @click="showInfoFun(row, index)" />
            </Tooltip>
            <Tooltip content="修改" placement="top">
              <Button v-if="permitIds.includes('dangjian_affairs_public_edit')" class="bianji" @click="editFun(row, index)" />
            </Tooltip>
            <Tooltip content="删除" placement="top">
              <Button v-if="permitIds.includes('dangjian_affairs_public_delete')" class="btnError" icon="ios-trash" @click="deleteFun(row, index)" />
            </Tooltip>
          </template>
        </Table>
      </Row>
...
import { getPartyWorkList, partyWorkDelete } from '@/api/zhsq/partyPublic/index'
...
import EditPartyPublic from './editPartyPublic/index'
export default {
  name: 'PartyPublic',
  components: {
    EditPartyPublic
  ...
  //  获取表格数据
    async getListData() {
      ...
      const { data } = await getPartyWorkList({
    ...
    editFun(row) {
      this.modelType = 'edit'
      this.editData = row
      this.editData.partBranchId = '' + this.editData.partBranchId
      this.editData.partyCommitteeId = '' + this.editData.partyCommitteeId
      this.modelTitle = '党务公开——编辑'
      this.modelStatus = true
    },
    // 查看详情
    showInfoFun(row) {
    ...
    deleteFun(row) {
      this.$Modal.confirm({
        title: '确认删除',
        content: '您确认要删除所点击选的党务?',
        loading: true,
        onOk: async() => {
          await partyWorkDelete(row.id)
          this.$Modal.remove()
