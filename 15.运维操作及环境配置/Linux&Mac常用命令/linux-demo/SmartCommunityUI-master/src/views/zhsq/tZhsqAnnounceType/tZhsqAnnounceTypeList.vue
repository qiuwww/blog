<style lang="less">
.tZhsqAnnounceTypeList {
}
</style>
<template>
  <div class="search" tZhsqAnnounceTypeList>
    <Card>
      <Row v-show="openSearch" @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="70" @submit.native.prevent>
          <Form-item label="名称类型">
            <Input type="text" v-model="searchForm.typeName" placeholder="请输入类型名称" clearable></Input>
          </Form-item>
...
<!--            </a>-->
      ...
      <Row class="operation">
        <Button @click="addTZhsqAnnounceType" type="primary" icon="md-add">新增</Button>
    ...
    <updateTZhsqAnnounceType v-model="updateShow" :TZhsqAnnounceTypeId="TZhsqAnnounceTypeId" :modalTitle="title"
                              v-on:handleSearch="getDataList"></updateTZhsqAnnounceType>
  </div>
</template>
<script>
import {
  deleteTZhsqAnnounceType,
  queryTZhsqAnnounceTypeList,
  updateTZhsqAnnounceType as update
} from '@/api/zhsq/tZhsqAnnounceType/tZhsqAnnounceType'
import {formartDate} from '@/api/tools/tool'
import updateTZhsqAnnounceType from './updateTZhsqAnnounceType'
...
export default {
  ...
  data() {
    return {
      ...
      search: '',
      data: [],
      columns: this.getTZhsqAnnounceTypeColumns(),
      ...
      updateShow: false,
      tableHeight: 200,
      colOptions: ["类型名称", "是否显示", "创建时间", "操作"],
      colSelect: ["类型名称", "是否显示", "创建时间", "操作"],
      csvData: [],
    }
  },
  methods: {
    ...
    addTZhsqAnnounceType() {
      ...
      this.TZhsqAnnounceTypeId = '';
    },
    //编辑
    handleEdit(row, index) {
      this.title = '编辑';
      this.TZhsqAnnounceTypeId = row.id.toString();
      this.updateShow = true;
    },
    //查看
    ...
    getDataList() {
      ...
      queryTZhsqAnnounceTypeList(this.searchForm).then(res => {
        if (res && res.success) {
          this.loading = false;
    ...
    patchdeleteData(ids) {
      ...
      deleteTZhsqAnnounceType({ids: ids}).then(res => {
    ...
    updateStatus(row){
      let _this = this;
      if (row.announceShow === '1') {
        row.announceShow = '0'
      } else {
        row.announceShow = '1'
      }
      let data = {
        announceShow: row.announceShow,
        id: row.id
      }
    ...
    checkboxChange: function (data) {
      var columnss = this.getTZhsqAnnounceTypeColumns();
  ...
  mounted() {
    this.getDataList();
    //this.tableHeight = window.innerHeight - this.$refs.tZhsqAnnounceTypeTable.$el.offsetTop - 270
  }
}
</script>
<style scoped="less">
