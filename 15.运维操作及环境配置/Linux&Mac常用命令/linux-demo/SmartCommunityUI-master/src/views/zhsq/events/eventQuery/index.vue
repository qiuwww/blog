<template>
  <el-row type="flex" class="app-warp eventQuery" justify="center">
        ...
        <Form ref="searchForm" inline :label-width="110" onSubmit="return false">
          ...
          <Form-item label="所属辖区">
            ...
            <TreeSelect v-model="gridId" v-bind:disabled="disabled" :data="gridPriority" v-width="200"
                        @on-change="onGridChange" v-if="gridIdShow"/>
          </Form-item>
          ...
          <Form-item :label-width="10">
            ...
            <!--                    @update="handleUpdate"/>-->
            <Input class="inputstyle" search enter-button placeholder="请输入标题、登记编号" v-model="queryStr"
                   @on-search="handleUpdate"/>
          </Form-item>
            ...
            <Row type="flex" justify="center" align="middle" :gutter="2">
              <Col span="14" justify="right">
                ...
                <Tooltip content="修改" transfer placement="top">
                  <Button class="bianji" @click="handleEdit(row)"></Button>
                </Tooltip>
                <Tooltip content="删除" transfer placement="top">
                  <Button class="btnError" icon="ios-trash" @click="handleDelete(row.id)"></Button>
                </Tooltip>
              </Col>
            </Row>
          </template>
    ...
    <eventDetail v-model="eventShow" :is-event-wait="isEventWait" :proc-inst-id="procInstId"
                 :t-zhsq-task-list-id="tEventListId" modal-title="事件详情"/>
    ...
    <eventUpdate v-model="eventUpdateShow" :TZhsqEventListId="TZhsqEventListId" :modalTitle="title"
                 v-on:handleSearch="handleSearch"></eventUpdate>
  </el-row>
</template>
...
<script>
  ...
  import editandAddEvent from './editandAddEvent'
