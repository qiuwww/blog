<template>
  <el-row type="flex" justify="center" class="app-warp hierarchicalReportingEvent">
    <el-col :span="24">
...
<!--    <taskDetail v-model="eventShow" :t-zhsq-task-list-id="tEventListId" modal-title="网格事件详情查看" />-->
    <eventDetail v-model="eventShow" :t-zhsq-task-list-id="tEventListId" modal-title="事件详情" />
  </el-row>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(id) {
        this.$confirm('事件删除之后，所有相关数据都将不可查询', {type: 'warning'})
          .then(async _ => {
