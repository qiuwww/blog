<template>
  <el-row type="flex" class="app-warp eventQuery" justify="center">
    <el-col :span="24">
      ...
      <Row>
        ...
        >
          ...
          <template slot="timeMore" slot-scope="{ row, index }">
            <Row type="flex" justify="center" align="middle" :gutter="2">
                <Tooltip :content="'要求办结时间:'+row.requirementDate" transfer placement="bottom">
                  <div class="timeMore">
                    <div class="yellow divClass" v-if="calculateTimeReDate(row.requirementDate)<5">
                      <svg-icon icon-class="timeMore1"/>
                      {{calculateTime(row.requirementDate)}}
                    </div>
                    <div class="red divClass" v-else>
                      <svg-icon icon-class="timeMore5"/>
                      {{calculateTime(row.requirementDate)}}
                    </div>
                  </div>
                </Tooltip>
    ...
    <eventDetail v-model="eventShow" :is-event-wait="isEventWait" :isEventSelf="false" :proc-inst-id="procInstId"
    ...
    methods: {
      ...
      handleGetAllEvents() {
        ...
        getAllLoad({
          ...
          status:"0,1,3",
          endTime:true,
          createUser: this.nowUserId,//当前用户负责的 待审批事件
        }).then(res => {
          this.tableLoading = false
       ...
       */
      calculateTime(time){
        if(time==undefined||time==null||time.toString().trim().length==0){
          return "";
        }
        try {
