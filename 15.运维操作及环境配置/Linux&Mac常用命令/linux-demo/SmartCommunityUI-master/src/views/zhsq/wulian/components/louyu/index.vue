    ...
    <el-col v-loading="loading">
      ...
      <el-row class="tablebox">
        ...
        >
          ...
          <el-table-column prop="doorName" label="云门禁名称" />
      ...
      <el-row type="flex" justify="center">
        <el-col :span="14">
          ...
          >
            ...
            <el-form-item prop="doorName" label="云门禁名称">
              <el-input v-model="form.doorName" :maxlength="20" />
            </el-form-item>
...
import {
  addBuildingIntercom,
  modifyBuildingIntercom,
  deleteBuildingIntercom,
  loadAllByQueryBuildingIntercom,
  addParkingBarrier,
...
export default {
  ...
  async created() {
    this.handleGetData();
    // this.indexPropertyList = await this.handleGetPropertyId("");
  ...
  watch: {
    async gridIdToGridId(n, o) {
      ...
      if (this.form && this.form.gridId && this.form.gridId != this.row.gridId) {
  ...
  methods: {
    //  删 除
    async delPostMethods(buildingIntercomId) {
      this.$confirm("确定删除吗？")
        .then(async (_) => {
          const o = await deleteBuildingIntercom({
            buildingIntercomId,
          });
          if (o.code == 200) {
            this.$notify({
    ...
    async handleGetData() {
      ...
      const { data } = await loadAllByQueryBuildingIntercom({
    ...
    addPostM() {
      this.$refs["form"].validate(async (v) => {
        const O = await addBuildingIntercom({
          ...this.form,
        });
        if (O && O.code == 200) {
          this.$notify({
    ...
    editPostM() {
      this.$refs["form"].validate(async (v) => {
        const O = await modifyBuildingIntercom({
          ...this.form,
        });
        if (O && O.code == 200) {
          this.$notify({
    ...
    deletePost(row, status) {
      this.$confirm(`请确认${status ? "停用" : "启用"}`, {
        ...
        .then(async (_) => {
          const O = await modifyBuildingIntercom({ ...row, status });
          if (O && O.code == 200) {
            this.$notify({
              title: O.msg,
              type: "success",
