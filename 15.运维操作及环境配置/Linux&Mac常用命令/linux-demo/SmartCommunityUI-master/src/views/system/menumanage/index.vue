  ...
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU"
      ...
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-show="form.type.toString() !== '2'" label="顶部菜单" prop="headName">
          <el-select v-model="form.headName" placeholder="请选择" style="width: 450px">
            <el-option
              v-for="(item, i)  in headNameOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value">
            </el-option>
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data"
      ...
      <el-table-column v-if="columns.visible('name')" :show-overflow-tooltip="true" label="菜单名称" width="125px"
                       prop="name"/>
      <el-table-column v-if="columns.visible('headName')" :show-overflow-tooltip="true" label="顶部菜单" width="100px"
                       prop="headName"/>
      ...
      <el-table-column v-if="columns.visible('permission')" :show-overflow-tooltip="true" prop="permission"
                       label="权限标识"/>
...
<script>
    ...
    export default {
        ...
        created(){
            getDictDataByType('menu_headName', {}).then(res => {
                if (res && res.success) {
                    this.headNameOptions = res.data;
                }
            });
        },
        methods: {
