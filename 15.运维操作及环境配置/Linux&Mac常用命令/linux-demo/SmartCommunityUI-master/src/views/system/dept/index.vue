...
<!--            </a>-->
      ...
      <Row class="operation">
        <Button @click="addDept" type="primary" icon="md-add">新增</Button>
...
<!--      </Row>-->
    ...
    <updateDept v-model="updateShow" :DeptId="DeptId" :modalTitle="title" v-on:handleSearch="handleSearch"></updateDept>
  </div>
</template>
<script>
  import {deleteByIds,edit, getDept} from '@/api/system/dept'
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectDate:null,
        enabledRadio:[],
        colOptions: ["名称", "状态", "创建时间", "操作"],
        colSelect: ["名称", "状态", "创建时间", "操作"],
      }
    },
    methods: {
      //列表上方更多操作
      ...
      getDataList() {
        ...
        getDept(this.searchForm).then(res => {
          this.loading = false;
          if (res) {
            //展开第一层
      ...
      checkboxChange: function (data) {
        var columnss = this.getDeptColumns();
    ...
    mounted() {
      ...
      //this.tableHeight = window.innerHeight - this.$refs.deptTable.$el.offsetTop - 270
    }
  }
</script>
<style scoped="less">
