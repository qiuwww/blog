<template>
  <div id="content" style="height: 100%;">
    ...
    <leftSearchList ref="child" />
    <div class="mabg">
      <span id="gpsnone" class="overview-gps" @click="setClass(flag)">
        <img src="/img/archives/gps-ic.png">
        <em>{{ gridPersonNumber }}</em>
      </span>

      <span id="gpsnone2" class="overview-gps" @click="setClass2(flag2)">
    ...
    <div class="top_title">
      <img
        ...
        @click="changluyou()"
      >
      <label class="top_title_font-size">智慧治理智慧平台</label>
    ...
    <div v-if="statisticsshow">
      <Statistics
        ...
        :class="{'statistics_background':Ismapchang,'statistics_background2':Ismapchang2}"
      />
    </div>
    <div
    ...
    >
      ...
      <span
        class="btn-zoology"
        :class="{'bottom_background':Btzoology}"
      ...
      <span
        class="btn-industry"
        :class="{'bottom_background':Btindustry}"
        @click="changdituindustry()"
      >
        <i>产业布局</i>
      </span>
      <span class="btn-radar" :class="{'bottom_background':BtGrid}" @click="changdituGrid()">
        <i>网格区域</i>
      </span>
      <span class="btn-line" :class="{'bottom_background':dianji}" @click="changditu()">
        <i>影像地图</i>
      </span>
      <!--      <span-->
      <!--        class="btn-column"-->
...
export default {
  ...
  methods: {
    ...
    // 查询设施信息
    seachbuild() {
      queryTBuildingArchivesAll({buildingType: '1'})
        .then((res) => {
          if (res && res.success) {
            res.data.forEach((element) => {
              if (element.location) {
    ...
    SaveGeoJSON2(element) {
      ...
      axios({
        ...
        .then((res) => {
          const pJSON = new GeoJSON().readFeatures(res.data)
