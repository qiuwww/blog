...
<template>
  <div class="search tBasicPolylineManagementList">
    <Card>
        ...
        <Col span="20" style="padding-left: 20px;">
          <Row v-show="openSearch" @keydown.enter.native="handleSearch">
            <Form ref="searchForm" :model="searchForm" inline :label-width="70">
              <Form-item label="线状名称">
                <Input v-model="searchForm.name" placeholder="请输入线状名称"/>
              </Form-item>
              ...
              <Form-item label="创建时间">
                <DatePicker v-model="selectDate" type="daterange" format="yyyy-MM-dd HH:mm:ss" clearable @on-change="selectDateRange"
          ...
          <Row class="operation">
            <Button @click="addTBasicPolylineManagement" type="primary" icon="md-add">新增</Button>
          ...
          <Row>
            <Table :loading="loading" border :columns="columns" sortable="custom" :data="data" @on-sort-change="changeSort"
              ...
              <template slot-scope="{ row, index }" slot="action">
                ...
                <Button type="primary" size="small" @click.native="handleEdit(row,index)">
                  <Icon type="ios-create-outline"/>
                  修改
                </Button>
                <Button type="error" size="small" @click.native="deleteData(row,index)">
    ...
    <updateTBasicPolylineManagement v-model="updateShow" :menuId="searchForm.menuId" :menuName="searchForm.menuName"
                                    :TBasicPolylineManagementId="TBasicPolylineManagementId" :modalTitle="title"
                                    v-on:handleSearch="getDataList"></updateTBasicPolylineManagement>

    <batchUpdateTBasicPolylineManagement v-model="batchUpdateShow" :menuId="searchForm.menuId" :menuName="searchForm.menuName"
                                    :TBasicPolylineManagementId="TBasicPolylineManagementId" modalTitle="批量修改"
                                    v-on:handleSearch="getDataList"></batchUpdateTBasicPolylineManagement>
    <lookTBasicPolylineManagement v-model="lookShow" :TBasicPolylineManagementId="TBasicPolylineManagementId"
                                  :modalTitle="title"></lookTBasicPolylineManagement>

    <!-- 地图弹窗 -->
    <mapLabel v-model="showMap" modalTitle="选择坐标点"  :menuId="menuIdMap.toString()" @resultPoint="getPosition" divId="tBasicPolylineManagementList"
              :objData="pointArr" :selectType="selectType"></mapLabel>

    <uploadShp v-model="showShpFile" :menuId="searchForm.menuId" :title="title" :menuName="searchForm.menuName" objectType="line"
               v-on:handleSearch="handleSearch"></uploadShp>
  </div>
</template>
<script>
    import {
        deleteTBasicPolylineManagement,
        queryTBasicPolylineManagementList,
        updateTBasicPolylineManagement as updateData
    } from '@/api/zhxc/tBasicPolylineManagement/tBasicPolylineManagement'
    import updateTBasicPolylineManagement from './updateTBasicPolylineManagement'
    import batchUpdateTBasicPolylineManagement from './batchUpdateTBasicPolylineManagement'
    import lookTBasicPolylineManagement from './lookTBasicPolylineManagement'
    ...
    export default {
        ...
        data() {
            return {
                ...
                csvData: [],
                menuIdMap:'',
                columns: this.getTBasicPolylineManagementColumns(),
                ...
                updateShow: false,
                tableHeight: 200,
                colOptions: ["线状名称", "菜单名称", "显示状态", "所属区域", "线状位置", "创建人员", "创建时间", "操作"],
                colSelect: ["线状名称", "菜单名称", "显示状态", "所属区域", "线状位置", "创建人员", "创建时间", "操作"],
                selectDate: null,
                lookShow: false,
                isChildren: false,
                showMap: false,
                ...
                tBasicPolylineManagementForm: null,
                rgbaColor: '',
        ...
        methods: {
            ...
            displayState(row) {
                let str = '';
                this.tBasicPolylineManagementForm = row;
                if (row.isShow == 0) {
                    str = '显示';
                    row.isShow = 1;
                ...
                this.$Modal.confirm({
                    ...
                    loading: true,
                    onOk: () => {
                        updateData(this.tBasicPolylineManagementForm).then(res => {
                            this.$Modal.remove();
                            if (res && res.code == 200) {
            ...
            labelMap(row) {
                ...
                this.tBasicPolylineManagementForm = row;
                // this.rgbaColor = this.colorTransform(this.tBasicPolylineManagementForm.polygonColor,this.tBasicPolylineManagementForm.alpha);
                this.showMap = true;
            },
            //获取坐标点
            getPosition(obj, type) {
                if (this.tBasicPolylineManagementForm) {
            ...
            addTBasicPolylineManagement() {
                ...
                if (!this.isChildren) {
                    ...
                    this.TBasicPolylineManagementId = '';
                } else {
            ...
            handleEdit(row, index) {
                this.title = '编辑';
                this.TBasicPolylineManagementId = row.id.toString();
                this.updateShow = true;
            },
            //查看
            ...
            getDataList() {
                ...
                queryTBasicPolylineManagementList(this.searchForm).then(res => {
                    if (res && res.success) {
                        this.loading = false;
            ...
            patchdeleteData(ids) {
                ...
                deleteTBasicPolylineManagement({ids: ids}).then(res => {
            ...
            //获取列表字段
            getTBasicPolylineManagementColumns() {
                return [
                    {
                        type: 'selection',
                        width: 60,
                    ...
                    },
                    {
                        ...
                        tooltip: true,
                        render: (h, params) => {
                            let isShow = params.row.isShow ? '隐藏' : '显示';
                            let datad = this.csvData.filter(item => item.id == params.row.id);
                            datad[0].position = isShow;
                            return h('div', [
            ...
            checkboxChange: function (data) {
                var columnss = this.getTBasicPolylineManagementColumns();
        ...
        mounted() {
            this.getDataList();
            this.getlist();
            //this.tableHeight = window.innerHeight - this.$refs.tBasicPolylineManagementTable.$el.offsetTop - 270
        },
    }
</script>
