  ...
  <Modal v-model="show" :title="editTitle" :mask-closable="false" class="tPointIcon">
    <Form ref="tPointIconForm" :model="tPointIconForm" :rules="tPointIconFormRule"
      ...
      <FormItem label="图片" prop="icon">
        <div class="ivu-upload ivu-upload-drag" v-on:click="handelup" style="width: 414px;">
          ...
          <img :src="'/tempfile'+tempImg" alt="" v-if="tempImg" class="upimg" >
          <div v-else>
      ...
      <Row>
        <FormItem label="是否显示" prop="isShow">
          <RadioGroup  v-model="tPointIconForm.isShow">
            <Radio v-bind:disabled="disabled"  v-for="(item,i) in isShowRadio" :key="i" :label="item.sort">{{item.label}}</Radio>
          </RadioGroup>
        </FormItem>
      </Row>
      <Row>
            ...
            getTPointIcon({id: this.TPointIconId}).then(res => {
              if (res) {
                if (res && res.code == 200) {
                  ...
                  this.tempImg=this.tPointIconForm.icon = res.data.icon;
