...
<template>
  <div class="search tPictureManagementList">
    <Card>
    ...
    <updatatPictureManagement v-model="updateShow" :TPointIconId="TPointIconId" :modalTitle="title"
                      v-on:handleSearch="getDataList"></updatatPictureManagement>
  </div>
</template>
<script>
  import {deleteTPointIcon, queryTPointIconList,updateTPointIcon as update} from '@/api/zhxc/tPointIcon/tPointIcon'
  import {formartDate} from '@/api/tools/tool'
  import updatatPictureManagement from './updatatPictureManagement'

  export default {
    components: {
      updatatPictureManagement
    },
    data() {
      return {
        drop: false,
        ...
        updateShow: false,
        tableHeight: 200,
        colOptions: ["图片地址", "备注", "是否显示", "创建时间", "操作"],
        colSelect: ["图片地址", "备注", "是否显示", "创建时间", "操作"],
        csvData: [],//导出用到的data
