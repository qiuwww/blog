...
<template>
  <div class="search tGridMapServiceList">
    <Card>
      <Row v-show="openSearch" @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="70">
          ...
          <FormItem label="服务类型" prop="type">
            <Select  v-model="searchForm.type" placeholder="请选择" clearable>
...
<!--            </a>-->
      ...
      <Row class="operation">
        <Button @click="addTGridMapService" type="primary" icon="md-add">新增</Button>
    ...
    <updateTGridMapService v-model="updateShow" :TGridMapServiceId="TGridMapServiceId" :modalTitle="title"
                           v-on:handleSearch="getDataList"></updateTGridMapService>
  </div>
</template>
<script>
  import {
    deleteTGridMapService,
    queryTGridMapServiceList,
    updateTGridMapService as update
  } from '@/api/zhxc/tGridMapService/tGridMapService'
  import {formartDate} from '@/api/tools/tool'
  import updateTGridMapService from './updateTGridMapService'
  import {getDictDataByType} from '@/api/index';

  export default {
    components: {
      updateTGridMapService
    },
    data() {
      return {
        drop: false,
        ...
        search: '',
        data: [],
        columns: this.getTGridMapServiceColumns(),
        ...
        updateShow: false,
        tableHeight: 200,
        colOptions: ["服务名称", "服务参数", "是否显示", "服务类型", "创建时间", "服务地址", "操作"],
        colSelect: ["服务名称", "服务参数", "是否显示", "服务类型", "创建时间", "服务地址", "操作"],
        typePriority: [],//服务类型集合
        csvData: [],//导出用到的data
      }
    },
    methods: {
      ...
      addTGridMapService() {
        ...
        this.TGridMapServiceId = '';
      },
      //编辑
      handleEdit(row, index) {
        this.title = '编辑';
        this.TGridMapServiceId = row.id.toString();
        this.updateShow = true;
      },
      //查看
      ...
      getDataList() {
        ...
        queryTGridMapServiceList(this.searchForm).then(res => {
          if (res && res.success) {
            this.loading = false;
      ...
      patchdeleteData(ids) {
        ...
        deleteTGridMapService({ids: ids}).then(res => {
      ...
      getTGridMapServiceColumns() {
        return [
            ...
            render: (h, params) => {
              ...
              return h('div', [
                h('i-switch', { //数据库1是已处理，0是未处理
                  props: {
                    type: 'primary',
                    value: params.row.checked,  //控制开关的打开或关闭状态，官网文档属性是value
                    trueValue: 1,
                    falseValue: 0
                  },
                  style: {
          ...
          },
          {
            ...
            render: (h, params) => {
              if (params.row.type) {
                var item = this.typePriority.filter(function (item) {
                  return item.value == params.row.type
                });
                if (item.length > 0) {
                  let dataF = this.csvData.filter(item => item.id == params.row.id);
                  dataF[0].type = item[0].label;
                }
                if (item && item[0]) {
                  return h("Tooltip", {
                    props: {
                      content: item[0].label,
                      placement: 'bottom',
                      transfer: true
                    }
                  }, item[0].label)
    ...
    mounted() {
      ...
      //this.tableHeight = window.innerHeight - this.$refs.tGridMapServiceTable.$el.offsetTop - 270
    }
  }
</script>
<style scoped="less">
