      ...
      <Row type="flex" justify="start">
        ...
        <Col span="20" style="padding-left: 20px">
          <Row v-show="openSearch" @keydown.enter.native="handleSearch">
            <Form ref="searchForm" :model="searchForm" inline :label-width="70">
              <Form-item label="菜单名称">
                <Input v-model="searchForm.name" placeholder="请输入菜单名称"/>
              </Form-item>
              ...
              <card :padding="10" class="cardWidth">
                ...
                <List item-layout="vertical">
                  <ListItem class="cardList">
                    <div class="listTitle">
                      <span class="titleName" :title="item.areaName+'——'+item.name">{{item.areaName+'——'+item.name}}</span>
                    </div>
          ...
          <Row type="flex" justify="end" class="page">
            <Page v-if="data!=''" :current="searchForm.pageNumber" :total="total" :page-size="searchForm.pageSize"
                  @on-change="changePage"
                  @on-page-size-change="changePageSize" :page-size-opts="[8,16,24]" size="small" show-total
                  show-elevator
                  show-sizer></Page>
          </Row>
          <Row>
            <img v-if="data==''" style="width: 64%;transform: translateX(18%);padding: 16%;" src="/img/tietu/noinfomation.png">
          </Row>
        </Col>
      </Row>
    </Card>
    <updataModel v-model="updateShow" :TPointIconId="searchForm.id" :areaId="searchForm.areaId"  :modalTitle="title"
                 v-on:handleSearch="getDataList"></updataModel>
  </div>
</template>
<script>
  import {deleteTTwoflanksList, queryTTwoflanksListList} from '@/api/zhxc/tTwoflanksList/tTwoflanksList'
  import areatree from "../components/areatree";
  import updataModel from './updataModel'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList() {
        this.loading = true;
        this.searchForm.id="";
        queryTTwoflanksListList(this.searchForm).then(res => {
          if (res && res.success) {
            this.loading = false;
      ...
      selectTree(v) {
        if (v.length > 0) {
          this.searchForm.areaId = v[0].id.toString();
      ...
      patchdeleteData(ids) {
        ...
        deleteTTwoflanksList({ids: ids}).then(res => {
