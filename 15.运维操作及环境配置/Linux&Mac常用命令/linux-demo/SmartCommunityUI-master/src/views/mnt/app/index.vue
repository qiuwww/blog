...
<template>
	<div class="search" mntAppList>
		<Card>
			<Row v-show="openSearch" @keydown.enter.native="handleSearch">
				<Form ref="searchForm" :model="searchForm" inline :label-width="70">
          <Form-item label="应用名称" prop="blurry">
			...
			<Row class="operation">
				<Button @click="addMntApp" type="primary" icon="md-add">新增</Button>
		...
		<updateMntApp v-model="updateShow" :MntAppId="MntAppId" :modalTitle="title" v-on:handleSearch="handleSearch"></updateMntApp>
	</div>
</template>
<script>
	import {deleteByIds, queryAppList} from '@/api/mnt/app'
	import { formartDate } from '@/api/tools/tool'
	import updateMntApp from './updateMntApp'
	export default {
		components: {
			updateMntApp
		},
		data() {
          ...
          name:null,
				...
				updateShow: false,
				tableHeight: 200,
				colOptions: ["应用名称","上传目录","部署路径","备份路径","应用端口","启动脚本","部署脚本","创建日期","操作"],
				colSelect: ["应用名称","上传目录","部署路径","备份路径","应用端口","启动脚本","部署脚本","创建日期","操作"],
			}
		},
		 methods: {
			 //列表上方更多操作
			...
			getDataList() {
				...
				queryAppList(this.searchForm).then(res => {
					if(res.content) {
						this.loading = false;
        ...
        var columnss =  this.getMntAppColumns();
