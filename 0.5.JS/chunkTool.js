...
function domScrollFlag(sel) {
  ...
  if (
    ...
    elem.children(":first").css("border-collapse") == "collapse"
  ) {
  ...
  flag.right =
    elem.get(0).scrollWidth > elem.innerWidth() + elem.scrollLeft()
      ? true
      : false;
  return flag;
}
...
sn.obj2str = function(o) {
  ...
  if (typeof o == "object") {
    if (o === null) return "null";
      ...
      for (var i = 0; i < o.length; i++) r.push(sn.obj2str(o[i]));
      r = "[" + r.join() + "]";
    }
    return r;
  }
  ...
  createArr: function(o, key) {
    ...
    for (var i in o) {
      if (typeof o[i] == "object") {
        new_key = key;
        if (
          (self._setting.k_filter && !self._setting.filterReg.test(i)) ||
          !self._setting.k_filter
        ) {
          if (key) {
        ...
        if (self._setting.v_flag) {
          if (
            (self._setting.v_filter && !self._setting.filterReg.test(i)) ||
            !self._setting.v_filter
          ) {
            if (key) {
              new_key = key + self._setting.split_char + i;
            } else {
