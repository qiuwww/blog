var koa = require('koa');
var parse = require('co-body');//Parse request bodies with generators inspired by Raynos/body.
var route = require('koa-route');
var views = require('co-views');
var webpack = require('webpack');
// 使用koa-webpack-dev-middleware来在koa当中开启webpack-dev-server。
...
var render = views('./src', { //渲染模板，使用数据对象来渲染
  ext: 'ejs'
});
...
io.on('connection', function(socket) {
  ...
  socket.on('disconnect', function() {
    ...
    io.emit('guest update',
      [...cache.nameListActive] //不定参数的格式是三个句点后跟代表所有不定参数的变量名。
    );
  });
